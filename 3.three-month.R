##############################
'3. Forecasting Models Recursively, 3-Month Ahead'
#Author: Will Winterbotham 
#Date: June 26th 2023
# 1/N ~ Forecasting with three lags, three steps ahead (Y_(t+3) = Y_t + Y_(t-1) + Y(t-2))
##############################
...
#use cbind to create df with 3 lags
df_lag <- cbind(y = df$GBRCPIALLMINMEI, lag(df,3), lag(df,4),lag(df,5))
df_lag <- df_lag[-c(1:5),]

#set colnames
colnames(df_lag) <- c("y",
                          paste0(colnames(df),"_L1"),
                          paste0(colnames(df),"_L2"),  
                          paste0(colnames(df),"_L3"))
...
system.time(
  for(ii in (train_length+1):t_length){
    ...
    #run model 
    ar_model <- lm(y ~ GBRCPIALLMINMEI_L1 + GBRCPIALLMINMEI_L2 + GBRCPIALLMINMEI_L3 , data = train_df)
...
#univariate forecast for GDP, in the SVAR format 
...
system.time(
  for(ii in (train_length+1):t_length){
    ...
    var_model <- lm(y ~ 
                      GBRCPIALLMINMEI_L1 + GBRCPIALLMINMEI_L2 + GBRCPIALLMINMEI_L3 + 
...
system.time(
  for(ii in (train_length+1):t_length){ 
    ...
    favar_model_df <- cbind(y = df_lag[1:(ii-1),1],pcs[,1:(no_pc)],
                            "GBRCPIALLMINMEI_L1" = df_lag[1:(ii-1),]$GBRCPIALLMINMEI_L1, 
                            "GBRCPIALLMINMEI_L2" = df_lag[1:(ii-1),]$GBRCPIALLMINMEI_L2,
                            "GBRCPIALLMINMEI_L3" = df_lag[1:(ii-1),]$GBRCPIALLMINMEI_L3)
    #run model 
    ...
    pca_test_x <- cbind(pca_test_x[,1:(no_pc)],
                        "GBRCPIALLMINMEI_L1" = df_lag$GBRCPIALLMINMEI_L1[ii],
                        "GBRCPIALLMINMEI_L2" = df_lag$GBRCPIALLMINMEI_L2[ii],
                        "GBRCPIALLMINMEI_L3" = df_lag$GBRCPIALLMINMEI_L3[ii]) #include all 3 lags
...
month_ahead <- 3   #3 month ahead forecast
...
system.time(
  for(ii in (train_length+1):t_length){
    ...
    #predict one step ahead
    nn_forecast[(ii-train_length)] <- fcast$mean[3]  #extract 3-ahead forecast 
  }
)
...
saveRDS(results_summary,"3.summary_results")
